...
console.log(
  '- This example is showing you how to create a user, create a transfer method and pay that user -'
);
console.log(
  '------------------------------------------------------------------------------------------------'
);
...
var server =
  process.env.HYPERWALLET_SERVER || 'https://sandbox.hyperwallet.com';
...
if (!username || !password || !programToken) {
  console.error(
    'Error: Please make sure that you have set the system environment variables HYPERWALLET_USERNAME, HYPERWALLET_PASSWORD and HYPERWALLET_PROGRAM_TOKEN!'
  );
  process.exit(-1);
}
 ...
 */
var hyperwallet = new Hyperwallet({
  username,
  password,
  server
});
